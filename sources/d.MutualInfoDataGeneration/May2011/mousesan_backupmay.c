...
double pumpkmr = 1.7;  // mM basal value. cannot be 0.
...
dt = 0.0013240282;
ft = 0.4619368025;
ikr_act = 0.4187771442;
ikr_inact = 0.9293840153;
ikr_inact2 = 0.1875749806;
iks_act = 0.0137976653;
fl12 = 0.9975345208;
dl12 = 0.0000035606;
fl13 = 0.9839103064;
dl13 = 0.0001657849;
...
nai = 8.1285272046;
ki = 139.8772222653;
resting = 0.7682630477;
open = 0.0000000762;
inactivated = 0.0000000218;
resting_inactivated = 0.2202704253;
...
gna_ttxs = 0.125*5.925e-05;
gna_ttxr = 0.125*5.925e-05;
gcal12 = 0.0010*4.0*1.9;
gcal13 = 0.0030*4.0*1.9;
gcat = 0.7*0.01862;
gh = 0.0057*0.95;
vhalf_gh = 106.8;
gkr = 0.775*0.002955;
...
pumpkmf = 0.000246; // mM basal value. cannot be 0.
pumpkmr = 1.7;  // mM cannot be 0.
...
// Kurata/Maltsev INaK
 if((filecounter==13)&&(whichParamForBif==0)){
   // ISO increases INaK. Demir increases INaKMax by 2 fold, Faber increases it by 30%, Berry gives a change of Kmnap but this can be left alone for now. We conservatively increase INaK by 10%.
   inak =  1.3*inakmax*(pow(ko,1.2)/(pow(kmkp,1.2)+pow(ko,1.2)))*(pow(nai,1.3)/(pow(kmnap,1.3)+pow(nai,1.3)))/(1.0+exp(-(v-ena+120.0)/30.0));
 }
 else
...
if((filecounter==2)&&(whichParamForBif==0))
 Jup  = 0.0;
  ...
  if((filecounter==13)&&(whichParamForBif==0)){
    pumpfactor = 1.0/3.3;
    Jup = Pup*(pow(cai/(pumpfactor*pumpkmf),pumphill) - pow(caup/(pumpfactor*pumpkmr),pumphill))/(1.0 + pow(cai/(pumpfactor*pumpkmf),pumphill) + pow(caup/(pumpfactor*pumpkmr),pumphill));
  }
  else
    {
...
if((filecounter==1)&&(whichParamForBif==0)){
 casub = 0.000050; // this has to be at cai or casub minimum.
 cai = 0.000050;
}
else
{
