...
gcal12 = 0.0010*4.0*1.5*0.5;
gcal13 = 0.0030*4.0*1.5*0.5;
gcat = 0.75*0.01862; 
gh = 0.0057; 
vhalf_gh = 106.8;
gkr = 0.8*0.002955*0.4; // E-4031
gto = 0.00492;
 kNaCa = 5.5;
Pup = 0.04;
ks = 1300000.0;
...
if((filecounter==13)&&(whichParamForBif==0))
 ical12 = 1.75*gcal12*fl12*dl12*fca*(v-ecal);
else
...
if((filecounter==13)&&(whichParamForBif==0))
 ical13 = 1.75*gcal13*fl13*dl13*fca*(v-ecal);
else
...
// Kurata/Maltsev INaK
 if((filecounter==13)&&(whichParamForBif==0)){
   ...
   inak =  1.1*inakmax*(pow(ko,1.2)/(pow(kmkp,1.2)+pow(ko,1.2)))*(pow(nai,1.3)/(pow(kmnap,1.3)+pow(nai,1.3)))/(1.0+exp(-(v-ena+120.0)/30.0));
 }
 else
...
if((filecounter==2)&&(whichParamForBif==0))
 Jup  = 0.0;
  ...
  if((filecounter==13)&&(whichParamForBif==0)){
    pumpfactor = 0.25;
    Jup = Pup*(pow(cai/(pumpfactor*pumpkmf),pumphill) - pow(caup/(pumpkmr),pumphill))/(1.0 + pow(cai/(pumpfactor*pumpkmf),pumphill) + pow(caup/(pumpkmr),pumphill));
  }
  else
    {
...
if((filecounter==0)&&(whichParamForBif==0)){
 nai = nai;
 ki = ki;
}
else
