/*
May 2011,
Alterations from March 2011:

1. gcal is augmented 1.75
2. gh is reduced 0.85
3. gCaT is reduced 0.85
4. new formulation of Jup.
*/
...
// #define pupornot 0 // 0 is pumpvmax (variable name is still pup), 1 is pup
...
#define R 8.314472 // mol^-1
#define T 310.5   
#define F 96.4846  // units: C/mol
...
/*
The conductance of gh from IV is 0.009 micro S. Mangoni has gh = 0.00324.
*/
...
#define kmnap 14.0 // mM AFFECTED BY ISO
#define kmkp 1.4 // mM
...
// #define ks 600000.0 // ms-1 for a frequency of 5 hertz or more
#define maxsr 15.0
#define minsr 1.0
#define hsrr 2.5
...
#define pumphill 2.0 // 2 seems to be alright
...
double pumpvmax = 0.04; // mM/ms: replacement of Pup: it is called Pup in the code


double pumpkmf = 0.000246; // mM basal value. cannot be 0.
double pumpkmr = 3.29;  // mM basal value. cannot be 0.
...
double Pup;
double Pup2; // this multiplies the Caup term, maybe. Not to be used.
double ks;
double gna_ttxs;
double gna_ttxr;
double gcat;
...
*/
 ...
 param_gcatsettozero = 0;

for(temp_counter = 1; temp_counter < count;temp_counter++){

  if(atoi(argv[temp_counter])<0||atoi(argv[temp_counter])>1){
 printf("bad input, exiting\n");
 exit(0);
}
}

if(count!=16){ printf("it takes integers as input\n"); exit(0);}
...
param_pumpkmf =  atoi(argv[13]);
param_pumpkmr =  atoi(argv[14]);

 param_gcatsettozero = atoi(argv[15]);
...
printf("%s %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d\n",argv[0],param_gna11,
 ...
 param_pup,
 param_ks,
 param_pumpkmf, param_pumpkmr,param_gcatsettozero
);
...
pumpkmf = 0.00008; // mM basal value. cannot be 0. If you increase this, the ICaT sensitivity is augmented.
// pumpkmf = 0.0006; // mM basal value. cannot be 0.
// pumpkmr = 3.1;  // mM cannot be 0.
pumpkmr = 4.5;  // mM cannot be 0.
...
// 13
 if(param_pumpkmf==1){
   do{
   pumpkmf = genrand_real1()*variation_multiply*0.1;
   }
   while(pumpkmf<=0.0);
 }
...
// 14
 ...
 if(param_pumpkmr==1){
   do{
   pumpkmr = genrand_real1()*variation_multiply*0.1;
   }
   while(pumpkmr<=0.0);
 }

 // 15
 if(param_gcatsettozero==1)
   gcat = 0.0;
...
ik1 = gk1*(ko/(ko + 0.228880))*(v - ek)*1.0/(1.0 + exp(0.070727*(v - ek)));
 else
...
if((filecounter==13)&&(whichParamForBif==0))
tau_ikr_act = 0.5*0.699821/(0.003596*exp((v)/15.339290) + 0.000177*exp(-(v)/25.868423));
else
...
if((filecounter==13)&&(whichParamForBif==0))
    ikr = 1.0*gkr*ikr_act*ikr_inact*(v - ek);
else
...
if((filecounter==13)&&(whichParamForBif==0))
 {
alpha_dl  = -28.39*(v+35.0+5)/(exp(-(v+35.0+5)/2.5)-1.0)-84.9*(v+5)/(exp(-0.208*(v+5))-1.0);
beta_dl   = 11.43*(v-5.0+5)/(exp(0.4*(v-5.0+5))-1.0);
 }
else
 {
...
if((filecounter==13)&&(whichParamForBif==0))
 {
dl13_inf = 1.0/(1+exp(-(v+13.5+5)/6.0));
fl13_inf = 1.0/(1+exp((v+35.0+5)/7.3));
tau_fl = (7.4 + 45.77*exp(-0.5*(v+28.1+5)*(v+28.1+5)/(11*11)));
 }
 else{
...
if((filecounter==13)&&(whichParamForBif==0))
 {
dl12_inf = 1.0/(1+exp(-(v+3.0+5)/5.0)); // Mangoni 2006: according to his email
fl12_inf = 1.0/(1+exp((v+36.0+5)/4.6)); // Mangoni 2006 35.9
 }
 else{
...
// Kurata/Maltsev INaK
 if((filecounter==13)&&(whichParamForBif==0)){
   // ISO increases INaK. Demir increases INaKMax by 2 fold, Faber increases it by 30%, Berry gives a change of Kmnap but this can be left alone for now. We conservatively increase INaK by 30%.
     inak = inakmax*(pow(ko,1.2)/(pow(kmkp,1.2)+pow(ko,1.2)))*(pow(nai,1.3)/(pow(kmnap-3,1.3)+pow(nai,1.3)))/(1.0+exp(-(v-ena+120.0)/30.0));
 }
 else
...
if((filecounter==2)&&(whichParamForBif==0))
 Jup  = 0.0;
  ...
  if((filecounter==13)&&(whichParamForBif==0)){
    pumpfactor = 0.5;
//      pumpfactor = 1.0;
    Jup = Pup*(pow(cai/(pumpfactor*pumpkmf),pumphill) - pow(caup/(2.0*pumpkmr),pumphill))/(1.0 + pow(cai/(pumpfactor*pumpkmf),pumphill) + pow(caup/(2.0*pumpkmr),pumphill));
  }
  else
    {
...
if((filecounter==1)&&(whichParamForBif==0)){
 casub = 0.000030; // this has to be at cai or casub minimum.
 cai = 0.000030;
}
else
{
...
if((filecounter==13)&&(whichParamForBif==0))
    cai = cai + ddt*((Jcadif*vsub-Jup*vup)/vi - (ConcCM*dFcmi + ConcTC*dFtc + ConcTMC*dFtmc)); // increase the volume
 else
...
if(screen_output==1){
     ...
     printf("%10.5f\t",cai_min[param_counter]);
     printf("%10.5f\t",cai_peak[param_counter]);
     printf("%10.5f\t",ki);
     printf("%10.5f\t",nai);
...
// 16
fprintf(outputcurrents,"%10.10f\t",cai*1000.0);
// 17
...
// 23
fprintf(outputcurrents,"%10.10f\t",casub*1000.0);
// 24
fprintf(outputcurrents,"%10.10f\t",caup*1000.0);
// 25
fprintf(outputcurrents,"%10.10f\t",carel*1000.0);
// 26
...
// note how pumpkmf and pumpkmr are at the END of the line. This will help with the sigmaplot.

if((one_or_two_pars==2)&&(number>=3)){
for (param_counter=number-3; param_counter< number; param_counter++)
fprintf( parameters,"%d\t%d\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%d\t%10.10f\t%10.10f\n",
filecounter,filecounter1,
	 (1000.0*gna_ttxs/capacitance),(1000.0*gna_ttxr/capacitance),
	 (gcal12/capacitance),(gcal13/capacitance),
	 (gcat/capacitance),(gh/capacitance),
	 vhalf_gh,(gkr/capacitance),
	 (gto/capacitance),(kNaCa/capacitance),
Pup,ks,
...
apd90[param_counter],cai_min[param_counter],
cai_peak[param_counter],ddr[param_counter],
	 top[param_counter],gcal_gcat, pumpkmf,pumpkmr);
}
else
{
fprintf( parameters,"%d\t%d\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%10.10f\t%d\t%10.10f\t%10.10f\n",
...
(1000.0*gna_ttxs/capacitance),(1000.0*gna_ttxr/capacitance),
	 ...
	 vhalf_gh,(gkr/capacitance),
	 (gto/capacitance),(kNaCa/capacitance),
Pup,ks,-100.0,-100.0,100.0,-100.0,-100.0,-100.0,-100.0,-100.0,-100.0,-100.0,-100.0,gcal_gcat, pumpkmf,pumpkmr);
}		   
fclose(parameters);
